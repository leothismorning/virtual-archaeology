...
#if WITH_EDITOR
TSharedPtr<const FVoxelTextureData> FVoxelTextureData::Create(const UVoxelTexture& VoxelTexture)
{
	...
	Data->Type = GetVoxelTextureDataType(*Data->Buffer);
	return Data;
}
#endif
...
#if WITH_EDITOR
TSharedPtr<FVoxelBuffer> FVoxelTextureData::CreateImpl(
	...
	int32& OutSizeY)
{
	...
	default:
	{
		...
		VOXEL_MESSAGE(Error, "{0}: unsupported texture format: {1}",
			VoxelTexture,
			UEnum::GetValueAsString(Source.GetFormat()));
