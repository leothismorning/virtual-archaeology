...
VOXEL_CONSOLE_VARIABLE(
	VOXEL_API, bool, GVoxelTraceBookmarks, false,
	"voxel.TraceBookmarks",
	"Emit Unreal Insight trace bookmarks when state is started/completed");
...
FVoxelState::FVoxelState(
	...
	, TaskContext(FVoxelTaskContext::Create(STATIC_FNAME("FVoxelState")))
{
	...
	if (GVoxelTraceBookmarks)
	{
		TRACE_BOOKMARK(TEXT("Voxel State %d - Start"), StateIndex);
	}

	FVoxelTaskScope Scope(*TaskContext);

	static const TArray<UScriptStruct*> Structs = INLINE_LAMBDA
	{
		TArray<UScriptStruct*> Result;
		for (UScriptStruct* Struct : GetDerivedStructs<FVoxelSubsystem>())
		{
			if (!FApp::CanEverRender() &&
				!MakeSharedStruct<FVoxelSubsystem>(Struct)->ShouldCreateOnServer())
			{
				continue;
			}
...
void FVoxelState::Render(FVoxelRuntime& Runtime)
{
	...
	if (GVoxelTraceBookmarks)
	{
		TRACE_BOOKMARK(TEXT("Voxel State %d - Render"), StateIndex);
	}
