...
VOXEL_CONSOLE_VARIABLE(
	VOXEL_API, bool, GVoxelShowChunkBounds, false,
	"voxel.render.ShowChunks",
	"Show bounds of leaf chunks");
...
void FVoxelRuntime::DrawDebug() const
{
	...
	if (OldState)
	{
		...
		if (GVoxelShowChunkBounds)
		{
			for (const FVoxelChunkKey& ChunkKey : OldState->GetSubsystem<FVoxelRenderSubsystem>().GetLeafChunkKeys())
			{
				const FVoxelBox Bounds = ChunkKey.GetBounds(
					OldState->Config->RenderChunkSize,
					OldState->Config->VoxelSize);

				FVoxelDebugDrawer(World)
				.LifeTime(0.1f)
				.Color(FLinearColor::Red)
				.DrawBox(Bounds, OldState->Config->LocalToWorld);
			}
		}
	}
