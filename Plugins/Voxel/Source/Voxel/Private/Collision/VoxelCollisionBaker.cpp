...
void AVoxelCollisionBaker::Render(const TVoxelMap<FIntVector, TSharedPtr<FVoxelCollisionChunk>>& ChunkKeyToChunk)
{
	...
	for (const auto& It : ChunkKeyToChunk)
	{
		...
		Component->SetRelativeScale3D(FVector(VoxelSize));
		Component->SetWorldLocation(FVector(It.Key) * ChunkSize * VoxelSize);
		Component->SetBodyInstance(Collision);
		Component->SetCollider(It.Value->Collider);
	}
