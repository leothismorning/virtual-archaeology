...
FVoxelInvokerCollisionSubsystem::FColliderRef FVoxelInvokerCollisionSubsystem::CreateColliderRef(const FIntVector& ChunkKey) const
{
	...
	FVoxelMesher Mesher(
		...
		*GetConfig().MegaMaterialProxy,
		...
		GetConfig().MesherSettings,
		false);

	Mesher.bRemoveEdges = true;
	Mesher.bQueryMetadata = false;

	const TSharedRef<FVoxelMesh> Mesh = Mesher.CreateMesh();
	...
	const TSharedRef<const FVoxelCollider>& Collider) const
{
	...
	Component.SetRelativeScale3D(FVector(Config.VoxelSize));
	Component.SetRelativeLocation(FVector(ChunkKey) * Config.CollisionChunkSize * Config.VoxelSize);
	Component.SetBodyInstance(Config.InvokerCollision);
