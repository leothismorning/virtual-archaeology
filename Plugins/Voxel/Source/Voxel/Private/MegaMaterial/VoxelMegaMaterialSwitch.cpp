...
int32 FVoxelMegaMaterialSwitch::CompileRVT(
...
void FVoxelMegaMaterialSwitch::AddVoxelHookDefine()
{
	...
	AddLine("#define VOXEL_HOOK (MATERIAL_VERTEX_PARAMETERS_VOXEL_VERSION == 5 && MATERIAL_PIXEL_PARAMETERS_VOXEL_VERSION == 7 && %s)", *TargetDefine);
}
...
void FVoxelMegaMaterialSwitch::ComputeDisplacementAndLayerMask()
{
...

	{
		...
		{
			const int32 PageToOffset_Texture = Compiler.TextureParameter(
				"VOXEL_PageToOffset_Texture",
				FVoxelTextureUtilities::GetDefaultTexture2D(),
				SAMPLERTYPE_Color);

			if (!ensureVoxelSlow(PageToOffset_Texture != -1))
			{
				return;
			}
			...
			AddLine("const int PageVertexOffset = asint(%s[GetTextureIndex_Log2(RootPageIndex, 8)].r);", *Translator.GetParameterCode(PageToOffset_Texture));
			AddLine("const FCluster Cluster = GetCluster(Parameters.Voxel_PageIndex, Parameters.Voxel_ClusterIndex);");
		}

		if (IsVertex())
		{
			AddLine("Parameters.Voxel_AttributesIndicesIndex = PageVertexOffset + (Cluster.GroupIndex + Parameters.Voxel_VertexIndex) * %d;", 1 + Switch.NumMetadatas);
		}
		else
		{
			AddLine("Parameters.Voxel_AttributesIndicesIndices = PageVertexOffset + (Cluster.GroupIndex + Parameters.Voxel_TriIndices) * %d;", 1 + Switch.NumMetadatas);
		}

		const int32 AttributesIndices_Texture = Compiler.TextureParameter(
			"VOXEL_AttributesIndices_Texture",
			FVoxelTextureUtilities::GetDefaultTexture2D(),
			SAMPLERTYPE_Color);

		const int32 AttributesIndices_TextureSizeLog2 = Compiler.ScalarParameter(
			"VOXEL_AttributesIndices_TextureSizeLog2",
			0.f);
		...
		if (!ensureVoxelSlow(AttributesIndices_Texture != -1) ||
			!ensureVoxelSlow(AttributesIndices_TextureSizeLog2 != -1) ||
			!ensureVoxelSlow(Materials_Texture != -1) ||
			!ensureVoxelSlow(Materials_TextureSizeLog2 != -1))
		{
			return;
		}
		...
		if (IsVertex())
		{
			AddLine(R"(
				GetVoxelMaterialLayers(
					Parameters.Voxel_AttributesIndicesIndex,
					%s,
					%s,
					%s,
					%s,
				...
				*Translator.GetParameterCode(AttributesIndices_Texture),
				*Translator.GetParameterCode(AttributesIndices_TextureSizeLog2),
		...
		else
		{
			AddLine(R"(
				GetVoxelMaterialLayers(
					...
					Parameters.Voxel_AttributesIndicesIndices,
					%s,
					%s,
					%s,
					%s,
