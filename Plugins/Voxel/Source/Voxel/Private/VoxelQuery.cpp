	...
	const FVoxelBox& Bounds) const
{
	...
	if (WeakLayer.Type == EVoxelLayerType::Height)
	{
		...
		return Layer->HasStamps(*this, Bounds, false);
	}
	else
	{
		...
		return Layer->HasStamps(*this, Bounds);
	}
}

bool FVoxelQuery::HasVolumeStamps(
	const FVoxelWeakStackLayer& WeakLayer,
	const FVoxelBox& Bounds) const
{
	VOXEL_FUNCTION_COUNTER();
	...
	return Layer->HasVolumeStamps(*this, Bounds);
}
...
FVoxelFloatBuffer FVoxelQuery::SampleHeightLayer(
	...
	const float Step,
	const FVoxelQueryCache& Cache) const
{
	...
	const FVoxelQueryCache& Cache) const
{
	...
	if (WeakLayer.Type == EVoxelLayerType::Height)
	{
		...
		if (ensureVoxelSlow(Layer))
		{
			Layer->SampleAsVolume(Cache, Query);
		}
	}
	else
	{
