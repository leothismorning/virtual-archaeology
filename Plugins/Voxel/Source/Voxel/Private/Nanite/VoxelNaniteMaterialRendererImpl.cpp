	...
	FSceneView& View)
{
	...
	if (QueuedData_RenderThread.IsValid())
	{
		...
		PageToOffset_Texture = QueuedData_RenderThread->PageToOffset_Texture;

		if (!ensure(PageToOffset_Texture) ||
			!ensure(QueuedData_RenderThread->PageToOffset.Num() > 0))
		{
		...
		RHIUpdateTexture2D_Safe(
			...
			FUpdateTextureRegion2D(
				...
				256),
			256 * sizeof(uint32),
			QueuedData_RenderThread->PageToOffset.View<uint8>());
	...
	if (UsedSurfaceTypes.Num() == 0 ||
		!ensureVoxelSlow(PageToOffset_Texture))
	{
		return;
	}
	...
	FRHICommandList& RHICmdList)
{
	...
	const TUniformBufferRef<FVoxelMaterialSelectionParameters> VoxelParameters = INLINE_LAMBDA
	{
		...
		Parameters.PageToOffset_Texture = PageToOffset_Texture;
		Parameters.VisBuffer64 = PassParameters.VisBuffer64;
