...
FVoxelMesh::FVoxelMesh(
	...
	const float MaxAbsDistance)
	: ChunkLOD(ChunkLOD)
	, ChunkOffset(ChunkOffset)
	, ChunkSize(ChunkSize)
	, MinAbsDistance(MaxAbsDistance)
{
	UpdateStats();
}
...
bool FVoxelMesh::ShouldGenerateChildren(const double VoxelSize) const
{
	if (!IsEmpty())
	{
		return true;
	...
	if (FVoxelUtilities::IsNaN(MinAbsDistance))
	{
		return false;
	}

	const double ScaledVoxelSize = VoxelSize * (int64(1) << ChunkLOD);

	return MinAbsDistance <= ScaledVoxelSize;
}
