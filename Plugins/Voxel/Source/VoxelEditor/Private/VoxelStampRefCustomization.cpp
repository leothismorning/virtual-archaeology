...
private:
		...
		IDetailPropertyRow* Row)
	{
		...
		{
			if (IDetailPropertyRow* DisableEditingRow = DetailInterface.AddExternalStructureProperty(StructProvider, GET_MEMBER_NAME_CHECKED(FVoxelHeightGraphStamp, bDisableEditingBlendMode)))
			{
		...
		{
			if (IDetailPropertyRow* DisableEditingRow = DetailInterface.AddExternalStructureProperty(StructProvider, GET_MEMBER_NAME_CHECKED(FVoxelVolumeGraphStamp, bDisableEditingBlendMode)))
			{
				AdditionalRow = DetailInterface.AddExternalStructureProperty(StructProvider, GET_MEMBER_NAME_CHECKED(FVoxelStamp, bApplyOnVoid));
				AdditionalRow->Visibility(EVisibility::Collapsed);
				DisableEditingRow->Visibility(EVisibility::Collapsed);
