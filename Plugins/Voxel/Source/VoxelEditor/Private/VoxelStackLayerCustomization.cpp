...
class FVoxelStackLayerCustomization
				...
				[
					PropertyCustomizationHelpers::MakeUseSelectedButton(
						MakeLambdaDelegate([this]
						{
							...
							if (Layer)
							{
								if (NewStack->HeightLayers.Contains(Layer) ||
									NewStack->VolumeLayers.Contains(Layer))
								{
									return;
								}
							}
							...
							{
								NewLayer = NewStack->VolumeLayers[0];
							}
							else if (NewStack->HeightLayers.Num() > 0)
							{
								NewLayer = NewStack->HeightLayers[0];
							}
			...
			[
				...
				.OnGetMenuContent_Lambda([this, ShouldFilterAsset]() -> TSharedRef<SWidget>
				{
					...
					return PropertyCustomizationHelpers::MakeAssetPickerWithMenu(
						Object,
						true,
						TArray<const UClass*>{ UVoxelHeightLayer::StaticClass(), UVoxelVolumeLayer::StaticClass() },
						{},
						{},
...
DEFINE_VOXEL_STRUCT_LAYOUT(FVoxelStackLayer, FVoxelStackLayerCustomization);