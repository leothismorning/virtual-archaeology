...
void FVoxelPlaceStampItem::Setup()
{
...
void SVoxelPlaceStampsTile::Construct(const FArguments& InArgs, const TSharedPtr<FVoxelPlaceStampItem>& InItem)
{
	WeakItem = InItem;
...
FReply SVoxelPlaceStampsTile::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	MenuBuilder.BeginSection("BasicOperations");
	{
		MenuBuilder.AddMenuEntry(
			...
			{
				const TSharedPtr<FVoxelPlaceStampItem> PinnedItem = WeakItem.Pin();
				if (!PinnedItem)
				{
					return;
				}
...
FReply SVoxelPlaceStampsTile::OnDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton))
	{
		if (ensure(Item->Asset.IsValid()))
		{
			...
			{
				return FReply::Handled().BeginDragDrop(FAssetDragDropOp::New(Item->Asset, AssetFactory));
			}
		}
	}
