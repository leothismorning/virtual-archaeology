...
void FVoxelGraphNode_Struct_Customization::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	for (const FName PinName : Node->Struct->PrivatePinsOrder)
	{
		if (const TSharedPtr<FVoxelNode::FVariadicPin> VariadicPin = Node->Struct->PrivateNameToVariadicPin.FindRef(PinName))
		{
			...
			TSharedRef<FVoxelGraphNodeVariadicPinCustomization> PinArrayCustomization = MakeShared<FVoxelGraphNodeVariadicPinCustomization>();
		...
		const auto SetupRowLambda = [this, ValueHandle, WeakNode = MakeVoxelObjectPtr(Node), PinName](FDetailWidgetRow& Row, const TSharedRef<SWidget>& ValueWidget)
		{
			Row
			.NameContent()
			[
			...
			[
				...
				[
					...
					{
						const UVoxelGraphNode_Struct* TargetNode = WeakNode.Resolve();
...
TSharedRef<SWidget> FVoxelGraphNode_Struct_Customization::CreateExposePinButton(const TVoxelObjectPtr<UVoxelGraphNode_Struct>& WeakNode, FName PinName) const
{
	return
		PropertyCustomizationHelpers::MakeVisibilityButton(
			FOnClicked::CreateLambda([WeakNode, PinName]() -> FReply
			{
				UVoxelGraphNode_Struct* TargetNode = WeakNode.Resolve();
				if (!ensure(TargetNode))
				{
