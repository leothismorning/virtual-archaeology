...
private:
	void GetActionColorIcon(FSlateIcon& OutIcon, FLinearColor& OutColor)
	{
		...
		case EVoxelGraphEditorInputType::Struct:
		{
			...
			const TSharedPtr<const FVoxelNode> Node = FVoxelNodeLibrary::FindNode(InputBinding.Struct.LoadSynchronous());
		...
		case EVoxelGraphEditorInputType::Function:
		{
			...
			const TSharedPtr<const FVoxelNode> Node = FVoxelNodeLibrary::FindNode(InputBinding.Function.LoadSynchronous());
			StructNodeAction.Node = Node;
