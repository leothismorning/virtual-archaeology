...
void SVoxelPinTypeComboBox::Construct(const FArguments& InArgs)
{
	...
	FVoxelPinTypeSet::OnChanged.Add(MakeWeakPtrDelegate(this, [this]
	{
		// Reset pin type selector to catch new user enums
		MenuContent = nullptr;
		PinTypeSelector = nullptr;
	}));
}
...
void SVoxelPinTypeComboBox::UpdateType(const FVoxelPinType& NewType)
{
	...
	SetToolTipText(FText::FromString(CachedInnerType.ToString()));
}
