...
void SVoxelCurveTemplateBar::Construct(const FArguments& InArgs, const TSharedRef<FCurveEditor>& InCurveEditor)
{
	...
	for (const FNiagaraCurveTemplate& CurveTemplate : GetDefault<UNiagaraEditorSettings>()->GetCurveTemplates())
	{
		...
		ToolBarBuilder.AddWidget(
			...
			.OnClicked(this, &SVoxelCurveTemplateBar::CurveTemplateClicked, TVoxelObjectPtr<UCurveFloat>(FloatCurveAsset))
...
FReply SVoxelCurveTemplateBar::CurveTemplateClicked(const TVoxelObjectPtr<UCurveFloat> WeakFloatCurveAsset) const
{
