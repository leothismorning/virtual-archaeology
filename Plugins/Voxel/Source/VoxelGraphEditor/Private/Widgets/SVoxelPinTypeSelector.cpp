...
void SVoxelPinTypeSelector::GetChildrenMatchingSearch(const FText& InSearchText)
{
	...
	const auto LookThroughList = [&](const TArray<TSharedPtr<FVoxelPinTypeSelectorRow>>& UnfilteredList, TArray<TSharedPtr<FVoxelPinTypeSelectorRow>>& OutFilteredList, auto& Lambda) -> bool
	{
		...
		for (const TSharedPtr<FVoxelPinTypeSelectorRow>& TypeRow : UnfilteredList)
		{
			...
			{
				TSharedRef<FVoxelPinTypeSelectorRow> NewCategory = MakeShared<FVoxelPinTypeSelectorRow>(TypeRow->Name, ValidChildren);
...
void SVoxelPinTypeSelector::FillTypesList()
{
	...
	AllowedTypes.Get().Iterate([&](const FVoxelPinType& Type)
	{
		if (!Type.IsBuffer())
		{
	...
	if (bAllowWildcard)
	{
		TypesList.Add(MakeShared<FVoxelPinTypeSelectorRow>(FVoxelPinType::Make<FVoxelWildcard>()));
	}
	...
	AllowedTypes.Get().Iterate([&](const FVoxelPinType& Type)
	{
		...
		{
			TypesList.Add(MakeShared<FVoxelPinTypeSelectorRow>(Type));
			return;
		}

		if (!Categories.Contains(TargetCategory))
		{
			Categories.Add(TargetCategory, MakeShared<FVoxelPinTypeSelectorRow>(TargetCategory));
		}

		Categories[TargetCategory]->Children.Add(MakeShared<FVoxelPinTypeSelectorRow>(Type));
	});
