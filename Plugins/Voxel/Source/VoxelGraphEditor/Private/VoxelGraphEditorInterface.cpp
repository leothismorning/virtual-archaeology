	...
	for (const UEdGraphNode* EdGraphNode : Nodes)
	{
		...
		SerializedNode.VoxelNode = INLINE_LAMBDA -> TVoxelInstancedStruct<FVoxelNode>
		{
			if (const UVoxelGraphNode_Struct* GraphNode = Cast<UVoxelGraphNode_Struct>(EdGraphNode))
			{
				...
				// Serialize to bytes to fully duplicate the struct, avoiding sharing of memory
				// which would cause issues with graph diffing
				// TODO REMOVE ONCE WE HAVE FVoxelInstancedStampRef

				TArray<uint8> Bytes;
				{
					FObjectWriter Writer(Bytes);
					ConstCast(GraphNode->Struct).Serialize(Writer);
				}

				FObjectReader Reader(Bytes);

				TVoxelInstancedStruct<FVoxelNode> Result;
