...
{
	if (!FPaths::DirectoryExists(GitRepoPath))
	{
		RunCmd("git clone git@github.com:VoxelPluginDev/VoxelPluginDocs", FPaths::GetPath(GitRepoPath));
	}

	RunCmd("git reset --hard", GitRepoPath);
	RunCmd("git pull", GitRepoPath);

	const FString APIDirectory = DocsPath / "resources" / "api";
	{
	...
	const FString SummaryPath = DocsPath / "SUMMARY.md";
	{
		FString Summary;
		...
		for (FString& Line : Lines)
		{
			...
			const TSharedRef<FNode> Node = MakeShared<FNode>();
	...
	RootNode->Children["Voxel Graph API"]->Children.Empty();
...

	{
		...
		{
			const FString Path = DocsPath / Node.Url;
		...
		Traverse(*RootNode->Children["Voxel Graph API"]);
	}
	...
	const TFunction<void()> OnFailure)
{
	...
	{
		Executable = PLATFORM_WINDOWS ? "git.exe" : "/usr/bin/git";
	...
	else
	{
		...
		{
			verify(FFileHelper::SaveStringToFile(CommandLine, TEXT("/tmp/script.sh")));

			Executable = "/bin/bash";
			FinalCommandLine = "/tmp/script.sh";
		}
	}
...
void UVoxelGenerateNodeDocsCommandlet::Generate(const bool bIsGatheringTexture)
{
	...
	for (const TSharedRef<const FVoxelNode>& Node : FVoxelNodeLibrary::GetNodes())
	{
		...
		SanitizedDisplayName.ReplaceInline(TEXT(":"), TEXT("_"));
		SanitizedDisplayName.ReplaceInline(TEXT("%"), TEXT("_"));
		SanitizedDisplayName.ReplaceInline(TEXT("("), TEXT("-"));
		SanitizedDisplayName.ReplaceInline(TEXT(")"), TEXT(""));
		SanitizedDisplayName.ToLowerInline();

		TArray<FString> Categories;
		CategoryString.ParseIntoArray(Categories, TEXT("|"));

		FString BasePath = DocsPath / "resources" / "api";
		for (const FString& Category : Categories)
		{
			BasePath /= Category.ToLower().Replace(TEXT(" "), TEXT("-"));
		}
		verify(IFileManager::Get().MakeDirectory(*BasePath, true));

		TSharedPtr<FNode> SummaryNode = RootNode->Children["Voxel Graph API"];
		FString SummaryPath = FString("resources") / "api";
		{
			for (const FString& Category : Categories)
			{
				SummaryPath /= Category.ToLower().Replace(TEXT(" "), TEXT("-"));

				TSharedPtr<FNode>& ChildNode = SummaryNode->Children.FindOrAdd(Category);
				if (!ChildNode)
				{
					ChildNode = MakeShared<FNode>();
		...
		{
			const TSharedRef<FNode> ChildNode = MakeShared<FNode>();
