...
FString FVoxelGraphEditorInputBinding::GetActionName() const
{
	...
	case EVoxelGraphEditorInputType::Struct:
	{
		...
		const TSharedPtr<const FVoxelNode> Node = FVoxelNodeLibrary::FindNode(Struct.LoadSynchronous());
		if (!Node)
		{
			return "INVALID";
		}
	...
	case EVoxelGraphEditorInputType::Function:
	{
		...
		const TSharedPtr<const FVoxelNode> Node = FVoxelNodeLibrary::FindNode(Function.LoadSynchronous());
		if (!Node)
		{
			return "INVALID";
		}
