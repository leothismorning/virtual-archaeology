...
TSharedRef<SWidget> SVoxelCurveEditor::MakeTopToolbar()
{
	...
	ToolBarBuilder.BeginSection("Tangents");
	{
		...
		AddButton(FCurveEditorCommands::Get().InterpolationCubicSmartAuto);
...
TSharedRef<SWidget> SVoxelCurveEditor::MakeTemplatesMenu()
{
	...
	for (const FNiagaraCurveTemplate& CurveTemplate : GetDefault<UNiagaraEditorSettings>()->GetCurveTemplates())
	{
		...
		MenuBuilder.AddWidget(
			...
			.OnClicked(this, &SVoxelCurveEditor::OnChangeCurveTemplate, TVoxelObjectPtr<UCurveFloat>(FloatCurveAsset))
...
FReply SVoxelCurveEditor::OnChangeCurveTemplate(const TVoxelObjectPtr<UCurveFloat> WeakFloatCurveAsset) const
{
	const UCurveFloat* FloatCurveAsset = WeakFloatCurveAsset.Resolve();
