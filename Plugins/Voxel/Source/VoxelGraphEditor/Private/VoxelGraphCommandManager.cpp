...
void FVoxelGraphCommandManager::MapActions() const
{
	...
	Commands.MapAction(FVoxelGraphCommands::Get().Compile, MakeLambdaDelegate([=, this]
	{
	...
	Commands.MapAction(FVoxelGraphCommands::Get().EnableStats, MakeLambdaDelegate([=, this]
	{
	...
	Commands.MapAction(FVoxelGraphCommands::Get().EnableRangeStats, MakeLambdaDelegate([=, this]
	{
	...
	Commands.MapAction(FVoxelGraphCommands::Get().FindInGraph, MakeLambdaDelegate([=, this]
	{
		Toolkit.GetSearchWidget()->FocusForUse({});
	}));

	Commands.MapAction(FVoxelGraphCommands::Get().FindInGraphs, MakeLambdaDelegate([=, this]
	{
	...
	Commands.MapAction(FVoxelGraphCommands::Get().ReconstructAllNodes, MakeLambdaDelegate([=, this]
	{
		Toolkit.Asset->ForeachTerminalGraph_NoInheritance([&](const UVoxelTerminalGraph& TerminalGraph)
		{
	...
	Commands.MapAction(FVoxelGraphCommands::Get().TogglePreview, MakeLambdaDelegate([=, this]
	{
		...
		UVoxelEdGraph* EdGraph = Cast<UVoxelEdGraph>(Node->GetGraph());

		if (Node->bEnablePreview)
		{
			const FVoxelTransaction Transaction(Node, "Stop previewing");
			Toolkit.Asset->Modify();
			Toolkit.Asset->bEnableNodeValueStats = false;
		...
		Toolkit.Asset->Modify();
		Toolkit.Asset->bEnableNodeValueStats = true;
	}));
	...
	Commands.MapAction(FGraphEditorCommands::Get().CreateComment, MakeLambdaDelegate([=, this]
	{
		...
		CommentAction.PerformAction(Graph, nullptr, GraphEditor->UE_506_SWITCH(GetPasteLocation, GetPasteLocation2f)());
	}));

	Commands.MapAction(FGraphEditorCommands::Get().SplitStructPin, MakeLambdaDelegate([=, this]
	{
		const TSharedPtr<SGraphEditor> GraphEditor = Toolkit.GetActiveGraphEditor();
		if (!ensure(GraphEditor))
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().RecombineStructPin, MakeLambdaDelegate([=, this]
	{
		const TSharedPtr<SGraphEditor> GraphEditor = Toolkit.GetActiveGraphEditor();
		if (!ensure(GraphEditor))
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().ResetPinToDefaultValue, MakeLambdaDelegate([=, this]
	{
		const TSharedPtr<SGraphEditor> GraphEditor = Toolkit.GetActiveGraphEditor();
		if (!ensure(GraphEditor))
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().FindReferences, MakeLambdaDelegate([=, this]
	{
	...
	Commands.MapAction(FGenericCommands::Get().Undo, MakeLambdaDelegate([this]
	{
		GEditor->UndoTransaction();
	}));

	Commands.MapAction(FGenericCommands::Get().Redo, MakeLambdaDelegate([this]
	{
		GEditor->RedoTransaction();
	}));

	Commands.MapAction(FGenericCommands::Get().SelectAll, MakeLambdaDelegate([this]
	{
	...
	Commands.MapAction(FGenericCommands::Get().Delete, MakeLambdaDelegate([this]
	{
	...
	Commands.MapAction(FGenericCommands::Get().Copy, MakeLambdaDelegate([this]
	{
	...
	Commands.MapAction(FGenericCommands::Get().Cut, MakeLambdaDelegate([this]
	{
		TSet<UEdGraphNode*> Nodes;
		{
			for (UEdGraphNode* Node : Toolkit.GetSelectedNodes())
	...
	Commands.MapAction(FGenericCommands::Get().Paste, MakeLambdaDelegate([this]
	{
	...
	Commands.MapAction(FGenericCommands::Get().Duplicate, MakeLambdaDelegate([this]
	{
		TSet<UEdGraphNode*> Nodes;
		{
			for (UEdGraphNode* Node : Toolkit.GetSelectedNodes())
	...
	Commands.MapAction(FGraphEditorCommands::Get().AlignNodesTop, MakeLambdaDelegate([this]
	{
		if (const TSharedPtr<SGraphEditor> ActiveGraphEditor = Toolkit.GetActiveGraphEditor())
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().AlignNodesMiddle, MakeLambdaDelegate([this]
	{
		if (const TSharedPtr<SGraphEditor> ActiveGraphEditor = Toolkit.GetActiveGraphEditor())
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().AlignNodesBottom, MakeLambdaDelegate([this]
	{
		if (const TSharedPtr<SGraphEditor> ActiveGraphEditor = Toolkit.GetActiveGraphEditor())
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().AlignNodesLeft, MakeLambdaDelegate([this]
	{
		if (const TSharedPtr<SGraphEditor> ActiveGraphEditor = Toolkit.GetActiveGraphEditor())
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().AlignNodesCenter, MakeLambdaDelegate([this]
	{
		if (const TSharedPtr<SGraphEditor> ActiveGraphEditor = Toolkit.GetActiveGraphEditor())
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().AlignNodesRight, MakeLambdaDelegate([this]
	{
		if (const TSharedPtr<SGraphEditor> ActiveGraphEditor = Toolkit.GetActiveGraphEditor())
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().StraightenConnections, MakeLambdaDelegate([this]
	{
		if (const TSharedPtr<SGraphEditor> ActiveGraphEditor = Toolkit.GetActiveGraphEditor())
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().DistributeNodesHorizontally, MakeLambdaDelegate([this]
	{
		if (const TSharedPtr<SGraphEditor> ActiveGraphEditor = Toolkit.GetActiveGraphEditor())
		{
	...
	Commands.MapAction(FGraphEditorCommands::Get().DistributeNodesVertically, MakeLambdaDelegate([this]
	{
		if (const TSharedPtr<SGraphEditor> ActiveGraphEditor = Toolkit.GetActiveGraphEditor())
		{
...
void FVoxelGraphCommandManager::PasteNodes(const FString& TextToImport) const
{
	...
	PasteNodes(GraphEditor->UE_506_SWITCH(GetPasteLocation, GetPasteLocation2f)(), TextToImport);
}
