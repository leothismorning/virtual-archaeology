...
void FVoxelMembersSchemaAction_Parameter::BuildContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	if (!ensure(Graph) ||
		!Graph->IsInheritedParameter(MemberGuid))
	{
		return;
	}
	...
	MenuBuilder.AddMenuEntry(
		...
		FUIAction
		{
			MakeLambdaDelegate([=, this]
			{
				...
				const UVoxelGraph* LastGraph = LocalGraph;
				for (const UVoxelGraph* BaseGraph : LocalGraph->GetBaseGraphs())
				{
...
FReply FVoxelMembersDragDropAction_Parameter::DroppedOnPin(
...
FReply FVoxelMembersDragDropAction_Parameter::DroppedOnNode(
	const UE_506_SWITCH(FVector2D, FVector2f&) ScreenPosition,
	const UE_506_SWITCH(FVector2D, FVector2f&) GraphPosition)
{
	UVoxelGraph* Graph = WeakGraph.Resolve();
	...
	UEdGraph& EdGraph)
{
	...
	if (&Graph->GetEditorTerminalGraph().GetEdGraph() != &EdGraph)
	{
		FVoxelGraphSchemaAction_NewParameterUsage Action;
		Action.Guid = ParameterGuid;
	...
	{
		FVoxelGraphSchemaAction_NewCustomizeParameter Action;
		Action.Guid = ParameterGuid;
		Action.PerformAction(&EdGraph, nullptr, GraphPosition, true);
		return FReply::Handled();
	}
	...
	MenuBuilder.BeginSection("VariableDroppedOn", FText::FromName(Parameter->Name));
	{
		MenuBuilder.AddMenuEntry(
			FText::FromString("Get " + Parameter->Name.ToString()),
			FText::FromString("Create Getter for parameter '" + Parameter->Name.ToString() + "'\n(Ctrl-drag to automatically create a getter)"),
			FSlateIcon(),
		...
		MenuBuilder.AddMenuEntry(
			...
			FText::FromString("Create Customization for parameter '" + Parameter->Name.ToString() + "'\n(Alt-drag to automatically create a customization node)"),
			FSlateIcon(),
			FUIAction(MakeLambdaDelegate([Guid = ParameterGuid, Position = GraphPosition, WeakGraph = MakeWeakObjectPtr(&EdGraph)]
			{
				UEdGraph* PinnedEdGraph = WeakGraph.Get();
