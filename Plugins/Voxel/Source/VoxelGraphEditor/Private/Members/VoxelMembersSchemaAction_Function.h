...
{
public:
	DRAG_DROP_OPERATOR_TYPE(FVoxelMembersDragDropAction_Function, FVoxelMembersDragDropAction_Base);
	...
	static TSharedRef<FVoxelMembersDragDropAction_Function> New(
		...
		const TVoxelObjectPtr<UVoxelTerminalGraph>& WeakTerminalGraph)
	{
		const TSharedRef<FVoxelMembersDragDropAction_Function> Operation = MakeShareable(new FVoxelMembersDragDropAction_Function(WeakTerminalGraph));
...
private:
	explicit FVoxelMembersDragDropAction_Function(const TVoxelObjectPtr<UVoxelTerminalGraph>& WeakTerminalGraph)
