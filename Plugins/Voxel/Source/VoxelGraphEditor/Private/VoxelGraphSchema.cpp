...
bool UVoxelGraphSchema::ConnectionCausesLoop(const UEdGraphPin* InputPin, const UEdGraphPin* OutputPin) const
{
	...
	{
		UEdGraphNode* Node = NodesToProcess.Pop(EAllowShrinking::No);
...
{
	// Actions are collected via SVoxelGraphEditorActionMenu
}
...
{
	const UVoxelEdGraph* VoxelGraph = Cast<UVoxelEdGraph>(HoverGraph);
	if (!VoxelGraph)
	{
		return;
	}
	...
	if (!Toolkit ||
		Toolkit->Asset->GetGraphTypeName() != "legacy")
	{
		return;
	}
...
void UVoxelGraphSchema::DroppedAssetsOnGraph(const TArray<FAssetData>& Assets, const UE_506_SWITCH(FVector2D, FVector2f)& GraphPosition, UEdGraph* EdGraph) const
{
	// TODO?
}
...
bool UVoxelGraphSchema::ShouldHidePinDefaultValue(UEdGraphPin* Pin) const
{
	...
	if (VoxelNode->ShouldHidePinDefaultValue(*Pin))
	{
		return true;
	}

	// Optional pins should be hidden
	ensure(!VoxelNode->IsPinOptional(*Pin));
