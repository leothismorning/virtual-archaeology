...
void UPCGVoxelStampSpawnerSettings::PostLoad()
{
	...
	if (NewTemplate)
	{
		NewTemplate->FixupProperties(nullptr);
	}
}
...
bool FPCGVoxelStampSpawnerElement::PrepareDataInternal(FPCGContext* InContext) const
{
	...
	if (bGenerateOutput)
	{
		...
		for (const FPCGTaggedData& Input : Inputs)
		{
			...
			OutputPointData->GetMutablePoints() = PointData->GetPointsCopy();
...
bool FPCGVoxelStampSpawnerElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	for (const FPCGTaggedData& Input : Inputs)
	{
		...
		{
			VOXEL_SCOPE_COUNTER_NUM("Build NewStamps", Points.Num());

			FVoxelUtilities::SetNum(NewStamps, Points.Num());
			...
			for (int32 Index = 0; Index < Points.Num(); Index++)
			{
				...
				NewTemplate->Transform = Point.Transform;
