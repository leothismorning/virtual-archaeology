...
void FVoxelNode_Generate2DPoints::Compute(const FVoxelGraphQuery Query) const
{
	...
	VOXEL_GRAPH_WAIT(Bounds, CellSize, Jitter, Seed)
	{
		...
		const TSharedRef<FVoxelVector2DBuffer> Positions = MakeShared<FVoxelVector2DBuffer>();
		...
		VOXEL_GRAPH_WAIT(CellSize, Ids, Positions, Heights, Bounds)
		{
			...
			const TSharedRef<FVoxelVectorBuffer> FilteredPositions = MakeShared<FVoxelVectorBuffer>();
			...
			const TSharedRef<FVoxelVector2DBuffer> GradientPositions = MakeShared<FVoxelVector2DBuffer>();
...
void FVoxelNode_Generate2DPoints::GeneratePositions(
	...
	FVoxelVector2DBuffer& OutPositions,
