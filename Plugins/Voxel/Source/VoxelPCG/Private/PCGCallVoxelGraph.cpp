	...
	const TVoxelMap<FName, TVoxelObjectPtr<UPCGPointData>>& PinToOutPointData)
{
...

	{
		...
		for (int32 Index = 0; Index < Points.Num(); Index++)
		{
			...
			Position.Set(Index, FVector3f(Point.Transform.GetTranslation()));
	...
	for (const FVoxelNode::TPinRef_Input<FVoxelPointSet>* Pin : Pins)
	{
...

		{
			...
			FVoxelUtilities::SetNum(NewPoints, NewPointSet->Num());
 			...
 			if (const FVoxelVectorBuffer* Position = NewPointSet->FindPtr<FVoxelVectorBuffer>(FVoxelPointAttributes::Position))
 			{
 				VOXEL_SCOPE_COUNTER("Position");

				for (int32 Index = 0; Index < NewPoints.Num(); Index++)
				{
					NewPoints[Index].Transform.SetTranslation(FVector((*Position)[Index]));
				}
 			}

 			if (const FVoxelQuaternionBuffer* Rotation = NewPointSet->FindPtr<FVoxelQuaternionBuffer>(FVoxelPointAttributes::Rotation))
 			{
 			...
 			if (const FVoxelVectorBuffer* Scale = NewPointSet->FindPtr<FVoxelVectorBuffer>(FVoxelPointAttributes::Scale))
 			{
 			...
 			if (const FVoxelFloatBuffer* Density = NewPointSet->FindPtr<FVoxelFloatBuffer>(FVoxelPointAttributes::Density))
 			{
 			...
 			if (const FVoxelVectorBuffer* BoundsMin = NewPointSet->FindPtr<FVoxelVectorBuffer>(FVoxelPointAttributes::BoundsMin))
 			{
 			...
 			if (const FVoxelVectorBuffer* BoundsMax = NewPointSet->FindPtr<FVoxelVectorBuffer>(FVoxelPointAttributes::BoundsMax))
 			{
 			...
 			if (const FVoxelLinearColorBuffer* Color = NewPointSet->FindPtr<FVoxelLinearColorBuffer>(FVoxelPointAttributes::Color))
 			{
 			...
 			if (const FVoxelFloatBuffer* Steepness = NewPointSet->FindPtr<FVoxelFloatBuffer>(FVoxelPointAttributes::Steepness))
 			{
 			...
 			if (const FVoxelPointIdBuffer* Id = NewPointSet->FindPtr<FVoxelPointIdBuffer>(FVoxelPointAttributes::Id))
 			{
