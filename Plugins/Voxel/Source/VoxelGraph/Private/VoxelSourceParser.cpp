...
void FVoxelSourceParser::Initialize()
{
	...
	{
		NodePathToPinToTooltip = {};
		FunctionPathToPropertyToDefault = {};
	});

	const FString Path = FVoxelUtilities::GetPlugin().GetBaseDir() / "Source" / "VoxelGraph" / "Private" / "VoxelSourceParser.json";

	INLINE_LAMBDA
	{
		FString JsonString;
		if (!ensureVoxelSlow(FFileHelper::LoadFileToString(JsonString, *Path)))
		{
			return;
		}
	...
	FVoxelUtilities::DelayedCall([Path, this]
	{
		VOXEL_FUNCTION_COUNTER();
		...
		FFileHelper::LoadFileToString(ExistingFile, *Path);
		...
		ensure(FFileHelper::SaveStringToFile(NewFile, *Path));
	});
}
