...
#include "Preview/VoxelOutputNode_Preview.h"
...
bool FVoxelGraphCompiler::LoadSerializedGraph(const FOnVoxelGraphChanged& OnTranslated)
{
	...
	// This needs to happen as late as possible as it requires graphs to be loaded
	{
		...
		for (const auto& It : SerializedGraph.NodeNameToNode)
		{
			...
			ConstCast(CallFunctionNode)->FixupPins(TerminalGraph.GetGraph(), OnTranslated);
		}
	}
...
void FVoxelGraphCompiler::AddPreviewNode()
{
	...
	const TSharedRef<FVoxelOutputNode_Preview> PreviewVoxelNode = MakeShared<FVoxelOutputNode_Preview>();
	...
	PreviewNode.NewInputPin(VOXEL_PIN_NAME(FVoxelOutputNode_Preview, ValuePin), PreviewedPin->Type);
...
void FVoxelGraphCompiler::RemoveNodesNotLinkedToQueryableNodes()
{
	...
	for (const FNode& Node : GetNodes())
	{
		...
		if (Node.GetVoxelNode().IsA<FVoxelOutputNode>() ||
			Node.GetVoxelNode().IsA<FVoxelNode_FunctionOutput>() ||
			Node.GetVoxelNode().IsA<FVoxelNode_CustomizeParameter>())
		{
