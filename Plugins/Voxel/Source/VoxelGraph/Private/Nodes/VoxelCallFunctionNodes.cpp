	...
	const FGuid& TerminalGraphGuid) const
{
	...
	const TConstVoxelArrayView<const FVoxelNode_FunctionOutput*> FunctionOutputs = TerminalGraph->GetFunctionOutputs();
	...
	const FVoxelCompiledGraph& CompiledGraph) const
{
	...
	const FVoxelOutputNode* OutputNode = TerminalGraph->GetMainOutputNode();
	if (!ensureVoxelSlow(OutputNode))
	{
