...
void FVoxelGraphTracker::FImpl::Broadcast(const UObject& Object)
{
	...
	if (GVoxelGraphTracker->NumMuteScopes > 0)
	{
		return;
	}
...
void FVoxelGraphTracker::NotifyEdGraphNodeChanged(const UEdGraphNode& EdGraphNode)
{
	...
	OnEdGraphNodeChangedImpl.Broadcast(EdGraphNode);
}
...
void FVoxelGraphTracker::NotifyBaseGraphChanged(UVoxelGraph& Graph)
{
	...
	for (UVoxelGraph* ChildGraph : Graph.GetChildGraphs_LoadedOnly())
	{
		...
		OnParameterValueChangedImpl.Broadcast(*ChildGraph);
...
void FVoxelGraphTracker::NotifyParameterValueChanged(UVoxelGraph& Graph)
{
	VOXEL_FUNCTION_COUNTER();
	LOG_VOXEL(Verbose, "NotifyParameterValueChanged %s", *Graph.GetPathName());

	for (const UVoxelGraph* ChildGraph : Graph.GetChildGraphs_LoadedOnly())
	{
		OnParameterValueChangedImpl.Broadcast(*ChildGraph);
	}
}
