...
struct FVoxelBufferStatics
{
	...
	void Initialize()
	{
		...
		const TVoxelArray<UScriptStruct*> Structs = GetDerivedStructs<FVoxelBuffer>();
		...
		{
			if (Struct == StaticStructFast<FVoxelBufferStruct>() ||
				Struct == StaticStructFast<FVoxelTerminalBuffer>() ||
				Struct == StaticStructFast<FVoxelGenericStructBuffer>())
			{
				continue;
			}
