...
void FVoxelParameterOverrides::Migrate()
{
	VOXEL_FUNCTION_COUNTER();

	for (auto& It : PathToValueOverride)
	{
		if (It.Key.Guids.Num() != 1)
		{
			continue;
		}
		const FGuid Guid = It.Key.Guids[0];

		if (!ensureVoxelSlow(!GuidToValueOverride.Contains(Guid)))
		{
			continue;
		}

		GuidToValueOverride.Add(Guid, It.Value);
	}
	PathToValueOverride.Empty();
}
