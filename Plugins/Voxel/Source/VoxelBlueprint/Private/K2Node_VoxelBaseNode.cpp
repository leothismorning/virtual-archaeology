...
void UK2Node_VoxelBaseNode::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
...
void UK2Node_VoxelBaseNode::AddConvertPinContextAction(UToolMenu* Menu, UGraphNodeContextMenuContext* Context, const FVoxelPinTypeSet& TypeSet) const
{
	...
	Section.AddSubMenu(
		...
		MakeLambdaDelegate([=, this](const FToolMenuContext&) -> TSharedRef<SWidget>
		{
			return
				SNew(SVoxelPinTypeSelector)
				.AllowedTypes(TypeSet)
				.OnTypeChanged_Lambda([=, this](const FVoxelPinType NewType)
				{
					if (!ensure(Context) ||
						!ensure(Context->Pin))
					{
